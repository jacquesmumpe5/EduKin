...
{
    /// <summary>
    /// Générateur de bulletins pour l'option 601 (Première et Deuxième année)
    /// Version moderne et optimisée
    /// </summary>
    public class B_Opt_601 : BulletinBase
    {
        private readonly Dictionary<string, string> _cellMapping;

        public B_Opt_601() : base()
        {
            ConfigurerChemins("601");
            _cellMapping = InitialiserMappingCellules();
        }

        /// <summary>
        /// Configure les chemins spécifiques à l'option 601
        /// </summary>
        protected override void ConfigurerChemins(string codeOption)
        {
            ...
            _outputPath = Path.Combine(userPath, "MonEcole", "Bulletin", codeOption, "Premiere");
            _pdfPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "MonEcole", "PdfExport", codeOption, "Premiere");
            _templatePath = "B_301_3.xlsx";
        }
        ...
        private Dictionary<string, string> InitialiserMappingCellules()
        {
            ...
            {
                {"1", "B13"}, {"2", "B14"}, {"3", "B15"}, {"4", "B16"},
                {"5", "B17"}, {"6", "B18"}, {"7", "B20"}, {"8", "B21"},
                {"9", "B22"}, {"10", "B23"}, {"11", "B24"}, {"12", "B26"},
        ...
        {
            worksheet.Cells["F4"].Value = $"ELEVE : {eleve.Nom} {eleve.Postnom} {eleve.Prenom}";
            worksheet.Cells["M4"].Value = $"SEXE : {eleve.Sexe}";
            worksheet.Cells["F5"].Value = $"NE(E) A : {eleve.LieuNaissance}";
            worksheet.Cells["M5"].Value = $"LE : {eleve.DateNaissance}";
        }
