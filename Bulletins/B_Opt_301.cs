...
{
    /// <summary>
    /// Générateur de bulletins pour l'option 301 (Première année)
    /// Version moderne et optimisée
    /// </summary>
    public class B_Opt_301 : BulletinBase
    {
        private readonly Dictionary<string, string> _cellMapping;

        public B_Opt_301() : base()
        {
            ConfigurerChemins("301");
            _cellMapping = InitialiserMappingCellules();
        }

        /// <summary>
        /// Configure les chemins spécifiques à l'option 301
        /// </summary>
        protected override void ConfigurerChemins(string codeOption)
        {
            ...
            _outputPath = Path.Combine(userPath, "MonEcole", "Bulletin", codeOption, "Première");
            _pdfPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "MonEcole", "PdfExport", codeOption, "Première");
            _templatePath = "B_301_1.xlsx";
        }
